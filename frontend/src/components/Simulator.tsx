...
export default function Simulator({ predictions }: SimulatorProps) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-miami-cyan/20 bg-tarmac-light/90 shadow-[0_0_30px_rgba(13,240,214,0.1)] backdrop-blur-md p-6 sm:p-8 group">
      ...
      <div className="grid gap-8 md:grid-cols-2 relative z-10">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div>
            ...
            <select 
              className="w-full rounded border border-white/20 bg-black/60 px-4 py-3 text-lg font-bold italic uppercase text-white outline-none focus:border-miami-cyan transition-colors"
          ...
          {/* Grid Slider - Upgraded to Vice Pink for contrast */}
          <div>
            <div className="flex justify-between items-end mb-2">
              <label className="block text-xs font-mono text-zinc-500 uppercase tracking-widest">Simulate Grid Position</label>
              <span className="text-xl font-black text-vice-pink italic drop-shadow-[0_0_8px_rgba(255,16,122,0.6)]">P{customGrid}</span>
            </div>
            <input 
              type="range" 
              min="1" max="20" 
              ...
              className="w-full h-2 bg-black rounded-lg appearance-none cursor-pointer accent-vice-pink"
            />
          ...
          {/* Run Button - Electric Cyan Glow */}
          <button 
            onClick={handleSimulate}
            disabled={isSimulating}
            className="mt-2 w-full rounded border border-miami-cyan bg-miami-cyan/10 py-4 font-black uppercase italic tracking-widest text-miami-cyan transition-all hover:bg-miami-cyan hover:text-black disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_0_15px_rgba(13,240,214,0.2)] hover:shadow-[0_0_25px_rgba(13,240,214,0.6)]"
          >
        ...
        <div className="rounded-xl border border-white/5 bg-black/40 p-6 flex flex-col justify-center shadow-inner">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {/* Simulated Stats - Highlighted in Vice Pink */}
            <div className="flex flex-col gap-1 border-l border-white/10 pl-4">
              <span className="text-[10px] uppercase tracking-wider text-vice-pink font-mono">Simulated Grid</span>
              <span className="text-xl font-black text-white italic">{simResult ? `P${simResult.simulated_grid}` : "--"}</span>
              <span className="text-[10px] uppercase tracking-wider text-vice-pink font-mono mt-2">New Predicted Finish</span>
              <span className={`text-4xl font-black italic drop-shadow-[0_0_15px_rgba(255,16,122,0.4)] ${simResult ? 'text-white' : 'text-zinc-700'}`}>
