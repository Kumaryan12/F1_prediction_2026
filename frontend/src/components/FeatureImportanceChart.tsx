    ...
    <div className="relative flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-tarmac-light/80 p-6 shadow-2xl backdrop-blur-md">
      ...
      <div className="mb-8 flex items-end justify-between border-b border-white/10 pb-4 relative z-10">
        <div>
          ...
          <p className="mt-1 text-xs font-mono uppercase tracking-[0.3em] text-miami-cyan">
            Feature Importance Weighting
          </p>
        </div>
        
        {/* Decorative Diagnostic Icon - Resprayed to Miami Cyan */}
        <div className="flex h-10 w-10 items-center justify-center rounded bg-miami-cyan/10 border border-miami-cyan/30 shadow-[0_0_15px_rgba(13,240,214,0.3)] animate-pulse">
          <svg className="w-5 h-5 text-miami-cyan" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          return (
            <div key={item.name || idx} className="group relative flex flex-col gap-2">
              <div className="flex justify-between items-end text-xs font-mono uppercase tracking-widest text-zinc-400 group-hover:text-white transition-colors duration-300">
                ...
                {/* Number changed to Vice Pink */}
                <span className="text-vice-pink font-bold drop-shadow-[0_0_8px_rgba(255,16,122,0.6)]">
                  {item.value ? (item.value * 100).toFixed(1) : "0.0"}%
                </span>
              </div>
              
              {/* The Glowing Bar - Electric Cyan to Vice Pink Gradient */}
              <div className="h-1.5 w-full overflow-hidden rounded-r-full bg-black/60 shadow-inner">
                <div
                  className="h-full rounded-r-full bg-gradient-to-r from-[rgba(13,240,214,0.8)] to-[#FF107A] shadow-[0_0_12px_rgba(255,16,122,0.6)] transition-all duration-1000 ease-out"
                  style={{ width: `${pct}%` }}
                />
              </div>
            </div>
